...
ApplicationVersionCall *ModDownloader::appVersionCall()
{
    ApplicationVersionCall *call = new ApplicationVersionCall(qnam_, this);
    return call;
}
...
static SteamModInfo parseSteamModInfo(const QByteArray rawData, const QString &debugContext, QString *errorInfo = nullptr)
{
    ...
    if (!json.object().contains("response") && !json.object().value("response").isObject())
    {
...
ApplicationVersionCall::ApplicationVersionCall(QNetworkAccessManager &qnam, QObject *parent)
    : QObject(parent), qnam_(qnam)
{}

static void parseGitHubRelease(const QByteArray rawData, const QString &debugContext, QString *version, QString *url, QString *errorInfo)
{
    QJsonParseError errors;
    const QJsonDocument json = QJsonDocument::fromJson(rawData, &errors);
    ...
    if (json.isNull() || !json.isObject())
    {
        qCWarning(steamAPI).noquote() << debugContext << "Invalid response from GitHub API:" << errors.errorString();
        if (errorInfo)
            *errorInfo = QStringLiteral("Invalid response from GitHub API: %1").arg(errors.errorString());
        return;
    }
    ...
    {
        qCWarning(steamAPI).noquote() << debugContext << "GitHub API did not respond with latest release.";
        if (errorInfo)
            *errorInfo = QStringLiteral("GitHub API did not respond with latest release");
        return;
    }
...
void ApplicationVersionCall::startLatest()
{
    const QString callDebugInfo = QString("LatestAppVersion");
    version_.clear();
    url_.clear();

    static const QString getLatestRelease = QStringLiteral("https://api.github.com/repos/qoala/InvisibleInc-ModManager/releases/latest");
    QNetworkRequest request(getLatestRelease);
    ...
    {
        qCDebug(steamAPI).noquote() << callDebugInfo << "Request End";
        parseGitHubRelease(reply->readAll(), callDebugInfo, &version_, &url_, &errorDetail_);
        reply->deleteLater();
        emit finished();
    });
}
